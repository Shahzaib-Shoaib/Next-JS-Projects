...
import {client,storefront,productQuery,productsQuery} from "@graphql";
...
export async function getStaticPaths() {
  const {data} =  await storefront(productsQuery(60));

  const paths = data.products.edges.map((product:any) => ({
    params: {
      handle: String(product.node.handle),
    },
  }));
...
//  const {data} = await storefront(productQuery({ handle: params.handle }));
const  {data}  = await storefront(singleQuery, { handle: params.handle });
  return {
    props: {
     // product: JSON.stringify(data?.productByHandle),
